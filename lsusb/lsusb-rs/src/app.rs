        ...
        let mut usb_device = {
            match device.open() {
                Ok(h) => match h.read_languages(timeout) {
                    Ok(l) => {
                        if !l.is_empty() {
                            Some(UsbDevice { handle: h, language: l[0], timeout })
                        } else {
                            None
                        }
                    },
...
fn print_config<T: UsbContext>(config_desc: &ConfigDescriptor, handle: &mut Option<UsbDevice<T>>) {
    ...
    println!("    bDescriptorType      {:3}", config_desc.descriptor_type());
...
fn print_interface<T: UsbContext>(interface_desc: &InterfaceDescriptor, handle: &mut Option<UsbDevice<T>>) {
    ...
    println!("      bDescriptorType      {:3}", interface_desc.descriptor_type());
...
fn print_endpoint(endpoint_desc: &EndpointDescriptor) {
    ...
    println!("        bDescriptorType      {:3}", endpoint_desc.descriptor_type());
    println!(
